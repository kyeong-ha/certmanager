    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div>
          ...
          <select id="search-filter" value={filterType} onChange={(e) => setFilterType(e.target.value)} className="border rounded p-2">
            <option value="user_name">이름</option>
            <option value="edu_name">교육원명</option>
            <option value="phone_number">전화번호</option>
            <option value="issue_number">발급번호</option>
          </select>
        </div>
        <div className="flex-1">
          <label htmlFor="search-value" className="block text-sm font-medium mb-1">검색어</label>
          <Input id="search-value" type="text" placeholder="검색어 입력" value={searchValue} onChange={(e) => setSearchValue(e.target.value)}/>
        </div>
